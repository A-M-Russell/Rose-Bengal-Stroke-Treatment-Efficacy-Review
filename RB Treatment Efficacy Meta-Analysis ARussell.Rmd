---
title: "Rose Bengal Treatment Efficacy Analysis"
...
---
All code for meta-analysis of treatment efficacy in Rose Bengal model of stroke. This analysis looks into which treatment classes are effecive in Rose Bengal stroke and how quality and publication bias may affect treatment efficacy. 
Requires two .csv excel files, Russell_Data_Extraction_Rose_Bengal_11.csv and Russell_Rose_Bengal_Quality_2.csv
Can also use the pre-run workspace file to save time running the code: RB_Treatment_Efficacy_Workspace_ARussell_5.RData
Markdown file started on 14/04/2022, last updated 22/07/2023

This analysis is the first half of a two-part analysis of Rose Bengal stroke literature. The second half assesses the control group data to determine what methodological variables affect infarct volume. The protocol for both these analyses is available on PROSPERO at http://www.crd.york.ac.uk/PROSPERO/display_record.php?ID=CRD42019115429
...
```{r subgroup_data_frames, include=FALSE}
...
```{r meta_regression_treatment_efficacy, warning=FALSE}
...
```{r Summed_quality_meta_regression, warning=FALSE}
...
Posthoc analyses for treatment efficacy (not used in thesis discussion but good to know)
```{r posthocs_treatment_setup, include=FALSE, eval=FALSE}
...
```{r posthocs_treatment_effiacy, include=FALSE, eval=FALSE}
# Select and correct
...
```{r efficacious_treatments_by_administration_time_analysis, include=FALSE}
#
...
```{r administration_time_alone_analysis, include=FALSE}
...
```{r efficacious_treatments_alone_analysis, include=FALSE}
...
```{r graph_efficacious_treatments_by_administration_time, include=FALSE, eval=FALSE}
...
```{r treatment_class_rain_plot, include=FALSE, eval=FALSE}
...
```{r treatment_comorbidity_rain_plot, include=FALSE, eval=FALSE}
...
```{r treatment_time_rain_plot, include=FALSE, eval=FALSE}
#make the raincloud plot
...
```{r treatment_measurement_time_rain_plot, include=FALSE, eval=FALSE}
#make the raincloud plot
...
```{r treatment_bodytemp_rain_plot, include=FALSE, eval=FALSE}
#make the raincloud plot
...
```{r treatment_randomisation_rain_plot, include=FALSE, eval=FALSE}
#make the raincloud plot
...
```{r treatment_blinded_allocation_rain_plot, include=FALSE, eval=FALSE}
#make the raincloud plot
...
```{r treatment_blinded_outcome_rain_plot, include=FALSE, eval=FALSE}
...
```{r treatment_power_calc_rain_plot, include=FALSE, eval=FALSE}
#make the raincloud plot for surgical level
...
```{r treatment_coefficients_quality_summed_meta, include=FALSE}
#coefficients for all variables in model
...
```{r treatment_quality_summed_residuals, include=FALSE}
...
```{r treatment_quality_summed_clean_plot_data,include=FALSE}
...
```{r treatment_quality_summed_rain_plot, include=FALSE, eval=FALSE}
#make the raincloud plot for treatment class
...
```{r treatment_funnel_plot_data, include=FALSE}
## Funnel Plots
...
#run the weighted meta regression of these publications for the funnel plot
...
```{r Funnel_plot_mean_effect_size, include=FALSE, eval=FALSE}
  
# Set the difference you are hoping to detect with delta_value. This section is using the "weighted mean effect size" 
summary_funnel_model<-summary(funnel_model)
delta_value<-summary_funnel_model$beta


# Calculate the values for the significance triangle bands on the graph
# First calculate the set of powers for 0.05 and 0.01 significance, then transform by 1/sqrt(total n)

#create variables for loop
n<- seq(4,90, by= 0.1)
delta<-numeric(length(n))
  
n.01<- seq(4,90, by= 0.1)
delta.01<-numeric(length(n))
  
 
  
for(i in 1:length(delta)){
   
  test.n<-power.t.test(n=n[i]/2, sd =1, power= 0.8, sig.level =0.05, strict = F)
  delta[i]<-test.n$delta
   
  test.n.01<-power.t.test(n=n[i]/2, sd =1, power= 0.8, sig.level =0.01, strict = F)
  delta.01[i]<-test.n.01$delta
  
  }

# dataframe for significance bands
Significance_threshold <- data.frame(cbind(1/sqrt(c(n,rev(n.01))), c(delta, rev(delta.01))))


# Calculate where the power bands should go based on the delta we are trying to detect
#create veriables for loop
power_test<-seq(1, 0.1, by =-0.1)
n_to_detect_delta<-numeric(length(power_test))


for(i in 1:length(power_test)){
    
  test_power<-power.t.test(delta=delta_value, sd =1, power= power_test[i], sig.level =0.05, strict = F)
  n_to_detect_delta[i]<-test_power$n
   
  }

#create dataframe for coloured power bands on the graph
reciprical_sqrt_n <- 1/sqrt(2*n_to_detect_delta)
power_polygon <- data.frame(reciprical_sqrt_n, power_test)
power_polygon$ymin <- c(power_polygon$reciprical_sqrt_n[-1],1)
  

#plot the graph
pdf("Funnel_plot_precision_power_1.pdf", height = 5, width = 7)
ggplot(df_publication_bias, aes(x = yi, y=1/sqrt(N_total))) +
  #add the coloured bands for power calculation thresholds
  geom_rect(data=power_polygon, inherit.aes = F,aes(xmin=-10, xmax=10, ymin=ymin,
            ymax=reciprical_sqrt_n, fill=(as.character(c(power_test[-1],0))))) +
  scale_fill_viridis_d(direction = -1, alpha = 0.75) + 
  #add the grey calculated significance threshold lines for 0.01<p<0.05
  geom_polygon(data=Significance_threshold, aes(x =X2, y=(X1)), fill = "darkgrey", alpha = 0.4)+ 
  geom_polygon(data=Significance_threshold, aes(x =-X2, y=(X1)), fill = "darkgrey", alpha = 0.4)+
  geom_vline(xintercept = summary_funnel_model$beta, linetype = 5, color = "#666666",size = 1.1) + #add a line for estimated effect size
  theme_classic() +#Strip all the themes down to black and white, no gridlines, no background
  labs(x = "SMD", 
       y="Precision estimate (1/sqrt(n))",
       fill = "Power") + #Label Axes + legend
  #theme(axis.line = element_line(size=1)) + #Make axis lines thicker
  theme(axis.title = element_text(size = 15), 
        axis.text.x = element_text(size=12, color="black"), 
        axis.text.y = element_text(size=12, color="black"),#Adjust axis font sizes and font colour
        axis.ticks=element_line(color="black", lineend = "square")) + #Make ticks black
  scale_x_continuous(expand = c(0, 0), limits = c(-10,10))+#Ensure axes join at 0,0 and set the y axis limits CAREFUL this cuts data
  coord_cartesian(ylim=c(0.465, 0.1)) +  #change the size of the plot without cutting any data points
  geom_point() + #plot the points on top of everything else
  scale_y_reverse() #reverse so good precision (low numbers) are on top
dev.off()


#check number of studies about 80% or 90% power
sum(1/sqrt(df_publication_bias$N_total)<power_polygon$reciprical_sqrt_n[power_polygon$power_test==0.8])
sum(1/sqrt(df_publication_bias$N_total)<power_polygon$reciprical_sqrt_n[power_polygon$power_test==0.9])
```

```{r Funnel_plot_theoretical_large_effect, include=FALSE, eval=FALSE}
# Set the difference you are hoping to detect with delta_value. This section is using a generally accepted "large" SMD of 0.8
delta_value2<--0.8

#significance threshold bands already calculated above

# Calculate where the power bands should go based on the delta we are trying to detect
#create veriables for loop
power_test2<-seq(1, 0.1, by =-0.1)
n_to_detect_delta2<-numeric(length(power_test2))


for(i in 1:length(power_test2)){
    
  test_power2<-power.t.test(delta=delta_value2, sd =1, power= power_test2[i], sig.level =0.05, strict = F)
  n_to_detect_delta2[i]<-test_power2$n
   
  }

#create dataframe for coloured power bands on the graph
reciprical_sqrt_n2 <- 1/sqrt(2*n_to_detect_delta2)
power_polygon2 <- data.frame(reciprical_sqrt_n2, power_test2)
power_polygon2$ymin <- c(power_polygon2$reciprical_sqrt_n2[-1],1)
  

#plot the graph
pdf("Funnel_plot_precision_power_largeSMD_1.pdf", height = 5, width = 7)
ggplot(df_publication_bias, aes(x = yi, y=1/sqrt(N_total))) +
  #add the coloured bands for power calculation thresholds
  geom_rect(data=power_polygon2, inherit.aes = F,aes(xmin=-10, xmax=10, ymin=ymin,
            ymax=reciprical_sqrt_n2, fill=(as.character(c(power_test2[-1],0))))) +
  scale_fill_viridis_d(direction = -1, alpha = 0.75) + 
  #add the grey calculated significance threshold lines for 0.01<p<0.05
  geom_polygon(data=Significance_threshold, aes(x =X2, y=(X1)), fill = "darkgrey", alpha = 0.4)+ 
  geom_polygon(data=Significance_threshold, aes(x =-X2, y=(X1)), fill = "darkgrey", alpha = 0.4)+
  geom_vline(xintercept = delta_value2, linetype = 5, color = "#666666",size = 1.1) + #add a line for estimated effect size
  theme_classic() +#Strip all the themes down to black and white, no gridlines, no background
  labs(x = "SMD", 
       y="Precision estimate (1/sqrt(n))",
       fill = "Power") + #Label Axes + legend
...
#check number of studies about 80% or 90% power
sum(1/sqrt(df_publication_bias$N_total)<power_polygon2$reciprical_sqrt_n2[power_polygon2$power_test2==0.8])
sum(1/sqrt(df_publication_bias$N_total)<power_polygon2$reciprical_sqrt_n2[power_polygon2$power_test==0.9])
```

```{r, power_calculations_per_study, include=FALSE, eval=FALSE}
#calculate the power per study

#make variables for mean SMD power and power with a theoretical 'large' SMD (-0.8)
df_publication_bias$Calculated_power_mean_SMD<-999
df_publication_bias$Calculated_power_theoretical_SMD<-999

#do power calculateion per study for mean SMD
for(i in 1:nrow(df_publication_bias)){
  
  power_test_results<-power.t.test(df_publication_bias$N_total[i]/2, sig.level = 0.05, delta = delta_value)
  df_publication_bias$Calculated_power_mean_SMD[i]<-power_test_results$power
  
}

#do power calculateion per study for theoretical
for(i in 1:nrow(df_publication_bias)){
  
  power_test_results2<-power.t.test(df_publication_bias$N_total[i]/2, sig.level = 0.05, delta = delta_value2)
  df_publication_bias$Calculated_power_theoretical_SMD[i]<-power_test_results2$power
  
}

#check number of studies above 80% power is the same as the funnel plot calculations
sum(df_publication_bias$Calculated_power_mean_SMD>0.8)
sum(df_publication_bias$Calculated_power_theoretical_SMD>0.8)

#summary stats for powers
summary(df_publication_bias$Calculated_power_mean_SMD)
summary(df_publication_bias$Calculated_power_theoretical_SMD)
